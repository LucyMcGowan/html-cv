...
header-includes:
  ...
  - \definecolor{boxcolor}{rgb}{0.52, 0.52, 0.51}
  - \pagestyle{fancy}
  - \fancyhf{}
  - \renewcommand{\headrulewidth}{0pt}
  - \fancyfoot[C]{\thepage}
  - '\fancyfoot[R]{\footnotesize \color{boxcolor} \textsc{Updated \today}}'
  ...
  - '\newcommand{\fn}[1]{\hfill \textsc{\small{\color{boxcolor} #1}}}'
...
::: {#second}
Office: 342 Manchester Hall  
Email: mcgowald@wfu.edu  
Website: \href{https://www.lucymcgowan.com}{www.lucymcgowan.com}  
...
\\begin{quote}\\href{<ifelse(!is.na(slides), slides, '')>}{``<ifelse(!is.na(slides), title, title)>.''}\\end{quote}",
.open = "<", .close = ">")
```
...
funding %>%
 filter(funded == "pending") %>%
  glue::glue_data("\\tabbedparagraph{<when>}{<funder>, \\textit{<what>}<ifelse(!is.na(PI), glue::glue(' (PI: {PI})'), '')>\\\\ Role: <role>  <ifelse(!is.na(amount), glue::glue('({amount})'), '')>} \n\n",
                  .open = "<", .close = ">")
}
